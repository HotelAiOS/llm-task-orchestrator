...
async def serve_frontend():
    """Serwuje główny plik HTML frontendu"""
...
async def process_with_events(request_id: str, prompt: str, decompose: bool) -> Dict:
    """Przetwarza zadanie z emitowaniem eventów - używa TaskOrchestrator.process_request()"""
    
    if decompose:
        # Informacja o dekompozycji
    ...
    # Użyj istniejącej metody orchestratora
    result = await orchestrator.process_request(prompt, decompose=decompose)
    
    # Jeśli było dekomponowane - emituj eventy dla subtasków
    if result.get('decomposed') and result.get('tasks'):
        # Emit: dekompozycja zakończona
        subtasks_info = [
            {
                "id": t['id'], 
                "type": t['type'], 
        ...
        # Emit eventy dla każdego subtaska (po fakcie, bo już są wykonane)
        for task in result['tasks']:
            ...
            await event_emitter.emit(TaskEvent.subtask_started(
                ...
                task['type'],
                task['prompt'],
                task.get('model', 'unknown')
            ))
            
            # Completed (lub failed jeśli był błąd)
            if task.get('result') and not task['result'].startswith('Błąd:'):
        ...
        # Emit: merging (jeśli było więcej niż 1 subtask)
        if len(result['tasks']) > 1:
            await event_emitter.emit(TaskEvent.task_merging(
                request_id,
                len(result['tasks'])
            ))
